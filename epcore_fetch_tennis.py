...
import logging
from robomaster import conn, robot, camera, led, action, vision
        ...
        if not self.checkBox_4.isChecked():
            ...
            if self.radioButton_2.isChecked():
                ...
                try:
                    ...
                    print("Robot Connection Failed. Reason: ", str(e))
                    ...
                    self.label_stacon_tip.setText("Confirm that the EP connection mode is wifi mode, press the red connection button on the main control, and scan the QR code on the right.")
...
class StcConnectThread(QThread):
    ...
    def run(self):
        logging.info("Waiting for the robot to scan for connections")
        global IsConnected
        try:
...
class RobotForm(QWidget, Ui_Form):
    def __init__(self, con_type):
        ...
        if not Isignore:
            ...
            sn_msg = "Robot Serial No: {0}".format(SN)
            ...
            #self.ep_gripper.sub_status(freq=5, callback=self.sub_data_handler_gripper)
    ...
    def start_grip_tennis(self):
        ...
        if IsGripTenis:
            ...
            self.pushButton_grip_tennis.setText("Start Grabbing")
            ...
            self.pushButton_grip_tennis.setText("Stop Grabbing")
    ...
    def sub_info_handler(self, batter_info):
        percent = batter_info
        msg = "Robot Power: {0}%.".format(percent)
    ...
    def sub_position_handler(self, position_info):
        ...
        x = round(x, 3)
        y = round(y, 3)
        xyz_msg = "Robot position: x:{0} y:{1} z:{2}".format(x, y, yaw)
    ...
    def sub_velocity_handler(self, velocity_info):
        ...
        vgy = round(vgy, 3)
        vgz = round(vgz, 3)
        vgxyz_msg = "Robot velocity: vgx:{0} vgy:{0} vgz:{0}".format(vgx)
    ...
    #Capture keyboard events, note that you need to set setFocus() in the control, 
    #otherwise the arrow keys and space bar cannot be captured
    ...
    # 键盘释放事件，发送停止运行指令
    #Keyboard release event, send stop command
    ...
    # 开线程执行抓起网球动作，保证画面不卡顿
    #Open a thread to execute the action of grabbing a tennis ball 
    #to ensure that the screen does not freeze
    ...
    def grip_tennis(self):
        ...
        try:
            # time.sleep(2)
            self.ep_arm.moveto(183, -71).wait_for_completed(timeout=2)
            # time.sleep(2)
            self.ep_gripper.close(power=50)
            time.sleep(1)
    ...
    # 开线程执行放下网球动作，保证画面不卡顿
    #Open the thread to execute the action of putting down the tennis ball 
    #to ensure that the screen does not freeze
    ...
    # 开线程执行播放音频提醒程序
    #Open a thread to execute the audio reminder program
        ...
        try:
            ...
            if IsGripTenis:
                ...
                if maxa < 150:
                    ...
                    # print("Time difference:",zeroyaw - time.perf_counter())
                    ...
                    #print("ball bx: ", bx, "by: ", by)
                    first_tennis_x = int(bx)
                    # print("The X position of the first identified ball center", first_tennis_x)
                    radius = int(radius)
                    img = cv2.circle(frame, center, radius, (0, 255, 0), 2)
                    # 利用已知的焦距、图像中的球的半径以及实际球的大小，求网球到相机间的距离
                    if radius != 0:
                        distance = f * ballr / radius
                    # print("Distance from tennis ball:", distance)
                    # 根据小球在图像中的位置，以视野中心点为目标，控制小球沿Y方向运动，使球保持于视野中心
                    if abs(target_x - bx) < 5:
                        
                        speedy = 0
                        #print("ball x position nearby and speedy : ",speedy)
                    else:
                        speedy = -self.pid_user(0.2, target_x, bx)
                        #print("ball x position far away and speedy: ", speedy)
                    if abs(target_y - by) < 5:
                        speedx = 0
                        #print("ball y position nearby and speedx: ", speedx)
                    else:
                        speedx = self.pid_user(0.2, target_y, by) / 100
                        #print("ball y position far away and speedx: ", speedx)
                    if speedx == 0 and speedy == 0:
                        stop_sound_threads = True
                        #print("Start Grabbing")
                        # print("The tasks that EP is performing: ", self.action.target)
                        IsGripTenis = False
                        self.pushButton_grip_tennis.setText("Start Grabbing")
                        self.grip_tennis_thread()
                    else:
                        # print("output speed：", speedx, speedy)
            ...
            if IsGetTenis:
                ...
                if len(self.markinfo) > 0:
                    ...
                    #print("self.markinfo[0].center[0]",self.markinfo[0].center[0])
                    #print("self.markinfo[0].pt2[1]",self.markinfo[0].pt2[1])
                    #print("self.markinfo[0].pt1[1]",self.markinfo[0].pt1[1])
                    #print("marker bx: ", bx, "by: ", by)
                    if abs(target_x - bx) < 10:
                        
                        speedy = 0
                        #print("marker x position nearby and speedy : ",speedy)
                    else:
                        
                        speedy = -self.pid_user(0.2, target_x, bx)
                        #print("marker x position far away and speedy : ",speedy)
                    if abs(target_height - by) < 10:
                        
                        speedx = 0
                        #print("marker y position nearby and speedx : ",speedx)
                    else:
                        
                        speedx = self.pid_user(0.2, target_height, by)/50
                        #print("marker y position far away and speedx : ",speedx)
                    if speedx == 0 and speedy == 0:
                        #print("Near to marker")
                        self.realease_tennis_thread()
                    else:
                        self.ep_chassis.drive_speed(x=speedx, y=0, z=speedy)
                        #print("Can not run to marker")
                else:
                    print("marker Not spotted")
            ...
            print("！！！！！Video browsing error!！！！！")
            print("Error reason:",e)
            # self.stopshow1()
            self.label_cam.clear()
            self.label_cam.setText("Wait for the screen to reconnect")
